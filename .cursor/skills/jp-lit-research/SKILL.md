...
| `foreign_book_lookup` | 「洋書を探したい」「NDLにない洋書の所蔵を確認したい」 | [bibliography-lookup.md](workflows/bibliography-lookup.md) |
| `union_catalog_lookup` | 「レア資料がどこにあるか」「全国の所蔵を探したい」 | [source-selection.md](heuristics/source-selection.md) |
| `subject_bibliography_search` | 「見たことも聞いたこともない本を探したい」「主題から本を探したい」 | [source-selection.md](heuristics/source-selection.md) |
...
**補助的に追加すること（見つからない時に優先）:**
4. 著者名の表記ゆれ（新字体/旧字体、姓と名の空白有無、ローマ字転写）を2種以上試す
5. 書名の副題有無・巻次表記（上/下、I/II、第1版/新版）を切り替える
6. ISBN がある場合は ISBN でも照合し、タイトル検索と独立に存在確認する
...
**方針提示時は、source ごとに「なぜそのDBを使うか」を1行で必ず書く。**
...
- `jp_lit_search` でメタデータ検索の再試行を先に行い、それでも不十分なら `jp_lit_search_fulltext` → ページ特定は `jp_lit_search_pages` → OCR確認は `jp_lit_get_text_coordinates`
...
### 0件・不十分ヒット時の再試行プロトコル（最大3ラウンド）

**Round 1（表記の修正）**
- 表記ゆれ、旧字、異体字、送り仮名、欧文転写を追加して同一 source で再検索

**Round 2（書誌の再構成）**
- 著者名、巻次、副題、刊年レンジ、ISBN の有無を切り替えて再検索

**Round 3（source の切替）**
- 入口DB（`ndl_search`）から個別DB（`ndl_catalog` / `cinii_books` / `cinii_articles` / `jstage_articles` / `irdb`）へ順次移る

3ラウンド実施後も有力候補が得られない場合は、未確認点（例: 出版年不詳、翻訳書の原題不明）を明示し、追加手掛かりをユーザーへ質問する。
...
- `total_after > 100` や複数 cache 統合後の傾向要約が主目的の場合は、必要に応じて要約専用のサブエージェントを使ってよい。ただし標準フローにはしない
- 返答例:
