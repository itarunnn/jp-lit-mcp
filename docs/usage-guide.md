...
こういうときは、まず保存済みの検索結果を再利用します。原則として、いきなり upstream に再検索しません。
対象は今の検索結果だけではありません。過去に保存した検索結果も横断検索して、必要なものだけ拾い直し、統合・差分抽出・共通集合化できます。
...
`jp_lit_search` の返り値に入る `cache.hit=true` は、その検索呼び出しで保存済み cache を再利用したことを示します。結果整理の前提になる原本は、ここから使います。
