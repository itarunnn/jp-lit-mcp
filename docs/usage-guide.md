...
`jp-lit-mcp` は検索 API だけでなく、同梱 Skills によって調査の進め方も補助します。分野や資料種によって、先に見るべき DB、検索語の作り方、確認済みと未確認の分け方は変わります。

調査中に「この分野では先に別の DB を見る」「この検索語は不自然」「この資料種では断定を避けたい」といった違和感があった場合は、[フィードバックの送り方](feedback.md) を参照してください。
