...
検索・取得系ツールの返り値に入る `cache.hit=true` は、その呼び出しで保存済み cache を再利用したことを示します。結果整理の原本として使うのは主に `jp_lit_search` の cache ですが、OCR・全文・図版・国書DB本文検索・典拠補助などの cached tool でも、同じ考え方で `cache.saved_at` と再取得導線を確認できます。`jp_lit_refine_results` は既定で整理後の先頭 30 件だけを返します。
...
jp_lit_export_view(
  ...
  params={ cache_keys=[...], combine="union" },
  export_all=true,
