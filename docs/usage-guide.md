...
このため、利用者が「デジコレ全文を調べて」「見落としがないか確認して」のように網羅性を求めている場合は、`jp_lit_search_fulltext` だけで調査を閉じないでください。MCP で拾える次世代デジタルライブラリー範囲を先に整理したうえで、必要に応じてデジコレ本体の公式検索画面をブラウザで検索し、ログインなし公開資料、館内限定資料、送信サービス限定資料を含むデジコレ本体側のヒット有無を別枠で確認します。最終回答では、API で確認した範囲と手動確認した範囲を分けて明記してください。
...
デジコレ本体の検索画面でヒットするのに `jp_lit_search_fulltext` では出ない資料がある場合は、検索漏れではなく収録範囲の差である可能性があります。これは館内限定・送信サービス限定資料だけでなく、デジコレ本体ではログインなしで閲覧できるが次世代デジタルライブラリー API の収録・検索対象ではない資料でも起こりえます。その場合は `ndl_digital` のメタデータ検索や公式画面での手動確認に切り替えてください。
...
| ツール                        | 何をするか                                                   | 使いどころ                                 | 注意点                                                                           |
| ----------------------------- | ------------------------------------------------------------ | ------------------------------------------ | -------------------------------------------------------------------------------- |
| `jp_lit_search_fulltext`      | 次世代デジタルライブラリーの OCR 全文を資料横断で検索します  | どの資料に語が出るか探す                   | デジコレ本体の全文検索 API ではありません。「ログインなしで閲覧可能」資料全体も対象ではありません |
