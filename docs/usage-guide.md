...
`優先` は読む順番や確認順を決めるための作業上の目安です。候補はフラットに並べるだけでなく、資料種別、出版社・媒体、シリーズ、著者属性、引用・書評状況、刊行時期、本文確認状況などを見て、`優先: 高` / `中` / `低` / `保留` のように仮に整理することがあります。ただし、出版社や媒体だけで文献の価値を確定しません。個人出版や一般向け新書は通常、二次文献・解説的媒体として慎重に扱いますが、著者が当事者である、刊行時期が論争や出来事と近い、本文が同時代証言や受容史を示す、といった条件が確認できる場合は、調査上の価値を個別に評価します。

二次情報も、何についての根拠かを分けて扱います。本文は内容把握の根拠、学会誌・専門誌の署名書評は受容や評価を見る根拠、DB 件名は主題推定の根拠、出版社紹介は内容紹介の根拠です。出版社紹介や検索スニペットを、本文読解や学術的評価の代わりにはしません。

個人 Web、ブログ、note などを根拠として使う場合は、最低限、プロフィール、所属・肩書、専門分野、researchmap / KAKEN / CiNii / ORCID / 大学ページ等へのリンク、記事内の出典やページ番号を確認します。プロフィール未確認なら、探索上の手がかりに留め、`次: 発信者プロフィール確認` のように残します。
