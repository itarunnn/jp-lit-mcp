...
- [基本の考え方](#基本の考え方)
  - [確認・本文・優先度・根拠の読み方](#確認本文優先度根拠の読み方)
  - [文献DBモードでは 1 回の返答で複数検索することがある](#文献dbモードでは-1-回の返答で複数検索することがある)
  - [調査後に残るもの](#調査後に残るもの)
  - [サブエージェント利用について](#サブエージェント利用について)
- [文献検証モード](#文献検証モード)
- [まず何を頼めばよいか](#まず何を頼めばよいか)
  - [文献を探したい](#文献を探したい)
  - [別名義や件名を確認したい](#別名義や件名を確認したい)
  - [研究課題・報告書 PDF を探したい](#研究課題報告書-pdf-を探したい)
  - [候補を外部書誌 DB で照合したい](#候補を外部書誌-db-で照合したい)
  - [書誌や所蔵を確認したい](#書誌や所蔵を確認したい)
  - [デジコレ全文から特定の語を探したい](#デジコレ全文から特定の語を探したい)
  - [古い表記や表記ゆれで探したい](#古い表記や表記ゆれで探したい)
  - [図版・挿絵を探したい](#図版挿絵を探したい)
  - [会議録を探したい](#会議録を探したい)
  - [検索結果を整理し直す](#検索結果を整理し直す)
  - [調査セッションの保存・エクスポート](#調査セッションの保存エクスポート)
...
- [調査行動へのフィードバック](#調査行動へのフィードバック)
- [目的別のよくある使い分け](#目的別のよくある使い分け)
...
検索語の展開では、必要に応じて NDL Authorities も参照し、別名義や件名標目を手がかりにします。
...
文献DBモードでは、未知の文献・資料・調べ方を探索する調査の場合、検索前に原則として次を参照して調査計画を作ります。タイトル・著者・ISBN・NCID・DOI などが明確な単純所蔵確認・書誌確認だけは省略できますが、0 件・ノイズ過多・初出/掲載号/一般誌記事探索に分岐したらここへ戻ります。
...
`jp-lit-mcp` は、LLM が文献の内容把握や学術的位置づけを最終決定するためのものではありません。エージェントは、本文を読んでいない文献でも、タイトル・要旨・目次・書評・出版社紹介・Web 上の断片から仮整理することがあります。その場合は、本文読解ではないことと、何を根拠にした整理かを示します。

検索結果の `availability.online=true` は、オンライン上にレコード、リンク、PDF、HTML、デジコレ、リポジトリページ等の入口があることを示します。これは、エージェントが本文や該当箇所を読んだことを意味しません。本文を読んだ場合だけ `確認: 本文` / `本文: 確認済み` と表示します。

| 表示 | 意味 |
| --- | --- |
| `仮整理` | 候補を探索上どう見ているか。本文未読でも、根拠を明示して使う |
| `優先` | 調査上の確認優先度。文献価値の確定ではない |
| `根拠` | 仮整理や優先度の材料。本文、要旨、目次、書評、出版社紹介など |
| `確認` | エージェントがどこまで確認したか |
| `本文` | 本文への入口・アクセス可否・読了状態 |
| `次` | 次に人間またはエージェントが確認すべきこと |

`確認` には、たとえば `確認: 書誌`、`確認: 要旨`、`確認: 目次`、`確認: 本文`、`確認: 引用経由` があります。`本文` には、`本文: 未確認`、`本文: オンライン入口あり未読`、`本文: オンライン入口なし`、`本文: アクセス制限`、`本文: 確認済み` があります。
...
優先: 中
根拠: 目次・出版社紹介
確認: 目次 / 本文: オンライン入口なし / 次: 所蔵確認
```

`優先` は読む順番や確認順を決めるための作業上の目安です。候補はフラットに並べるだけでなく、資料種別、出版社・媒体、シリーズ、著者属性、引用・書評状況、刊行時期、本文確認状況などを見て、`優先: 高` / `中` / `低` / `保留` のように仮に整理することがあります。ただし、出版社や媒体だけで文献の価値を確定しません。

候補に DOI、タイトル、著者、刊行年がある場合は、`jp_lit_enrich_record` で Crossref / OpenAlex による外部書誌照合を行えます。これは候補が実在するか、DOI が一致するか、同名候補と混ざっていないかを見る補助です。照合ヒットや引用数は読む順番の材料にはできますが、本文到達性・本文読了・研究上の重要度を保証しません。日本語人文系では Crossref / OpenAlex に未収録でも重要な論文・図書・紀要・地域資料が普通にあります。

自費出版、個人出版支援、オンデマンド出版は、編集・査読・流通経路や典拠の確認が難しいことがあるため、主題に一致するだけで `優先: 高` にはせず、著者属性、典拠の扱い、引用状況、専門的な書評・批判・応答、本文確認状況を見て、`中` または `保留` から評価します。

二次情報も、何についての根拠かを分けて扱います。本文は内容把握の根拠、学会誌・研究会誌・紀要・専門誌の署名書評、批判、応答、紹介論文は受容や評価を見る根拠、DB 件名は主題推定の根拠、出版社紹介は内容紹介の根拠です。学術的な書評や応答は単なる周辺情報として落とさず、主要文献の位置づけを確認する材料として候補に残します。出版社紹介や検索スニペットを、本文読解や学術的評価の代わりにはしません。
...
文献DBモードの回答では、末尾に `検索概要`、`今回の確認範囲`、`調査ログ` が付くことがあります。`調査ログ` の `total` は検索元が返した総ヒット数、`取得件数` は実際に取得した件数、`抽出件数` は回答に使う候補として選別した件数です。取得件数と抽出件数が違う場合は、全件を読んだわけではなく、確認範囲を区切って候補を出していると読んでください。

### 調査後に残るもの

文献DBモードでは、調査後の情報を次の層に分けて扱います。

| 層 | 用途 | 主な読者 |
| --- | --- | --- |
| `cache` | 検索結果・取得 payload の保管。再整理や再検証の材料 | MCP / エージェント |
| `session trace` | 調査目的、source 選択、検索試行、採用・保留・除外理由、未確認事項、次アクションの記録 | エージェント / 後続処理 |
| `最終回答` | その場でユーザーに返す検索概要、候補、今回の確認範囲、調査ログ | ユーザー |
| `調査トレース報告 / handoff report` | session trace と cache を読める形にした判断用レポート | 主エージェント / 人間 |

session trace は復元用、handoff report は判断用です。cache は生データ保管であり、最終回答や調査トレース報告の代わりではありません。

サブエージェント使用時は必須として、調査トレース報告 / handoff report を返します。単独エージェント時は必要に応じて作成します。対象は、長い調査、後で研究ノート化したい調査、件数解釈や採否判断が重要な調査、またはユーザーが依頼した場合です。作成しなかった場合でも、cache / session trace が残っていれば、後から詳細版 report として再構成できます。
...
サブエージェントを使う場合は、対象 `cache_key` / `session_id`、担当範囲、使ってよい source、返してほしい調査トレース報告の形式を固定します。各担当は検索試行、採用/保留/除外理由、本文確認範囲、未確認事項を trace に残し、主エージェントへ brief summary ではなく調査トレース報告 / handoff report を返します。

主エージェントはまず handoff report を読み、必要な場合だけ trace / cache に戻って確認します。そのうえで重複・競合・優先度を整理し、最終判断と回答を統合する single writer になります。`jp-lit-mcp` と同梱 Skills は、単独のエージェントでも調査を進められるように設計しています。
...
分類から探索語を作りたい場合は、分類記号から件名標目を探せます。件名が弱い古い図書や、最初の検索語が分からない分野では、分類から入ると未知の本を見つけやすくなります。
...
研究テーマの概況、科研費課題、研究成果報告書 PDF、成果リストの手がかりを探す場合は `jp_lit_search_kaken_projects` を使います。

```text
文献DBで、近世出版文化に関する KAKEN の研究課題と報告書 PDF を手がかりに、次に探す文献検索語を出してください。
```

KAKEN は研究課題と研究成果報告書 PDF の入口です。成果リストに論文・図書・学会発表が出ることがありますが、それらは文献候補として扱い、本文や書誌の確定は CiNii / J-STAGE / IRDB / NDL などで確認します。

`detail_limit` を小さくすると詳細ページ取得を抑えられます。`report_pdf_status="not_checked"` は報告書 PDF が存在しないという意味ではなく、詳細取得を省略した状態です。

---

### 候補を外部書誌 DB で照合したい

NDL / CiNii / J-STAGE / IRDB などで見つけた単一候補について、DOI や title/author/year の一致を補強したい場合は `jp_lit_enrich_record` を使います。
...
| `jp_lit_search_fulltext`      | 次世代デジタルライブラリーの OCR 全文から資料を横断検索する |
| `jp_lit_search_pages`         | ヒットした資料の中で、どのページに語があるか特定する       |
| `jp_lit_get_text_coordinates` | 特定ページの OCR テキスト・座標・ページ画像 URL を取得する |

`jp_lit_search_fulltext` はデジコレ本体の検索画面そのものを API 化したものではありません。上流は次世代デジタルライブラリー API なので、デジコレ本体の全文検索画面で見える館内限定・送信サービス限定資料の OCR ヒットを網羅しません。`content_access.manual_viewing` は、MCP から本文を自動取得できない資料を人間が公式画面で読むための導線であり、MCP がデジコレ本体の全文検索結果を取得済みであることを意味しません。

このため、利用者が「デジコレ全文を調べて」「見落としがないか確認して」のように網羅性を求めている場合は、`jp_lit_search_fulltext` だけで調査を閉じないでください。MCP で拾える次世代デジタルライブラリー範囲を先に整理したうえで、必要に応じてデジコレ本体の公式検索画面をブラウザで検索し、館内限定・送信サービス限定資料のヒット有無を別枠で確認します。最終回答では、API で確認した範囲と手動確認した範囲を分けて明記してください。
...
デジコレ本体の検索画面でヒットするのに `jp_lit_search_fulltext` では出ない資料がある場合は、検索漏れではなく収録範囲の差である可能性があります。その場合は `ndl_digital` のメタデータ検索や公式画面での手動確認に切り替えてください。

逆に、`jp_lit_search_fulltext` で 0 件でも、デジコレ本体の全文検索画面まで確認していないなら「デジコレ全文検索で 0 件」とは書かず、「MCP の次世代デジタルライブラリー OCR 検索では 0 件。デジコレ本体の公式検索画面は未確認」のように範囲を限定して書きます。
...
- `jp_lit_prune_cache`
  - 古い cache の削除候補を確認し、必要な分だけ削除する
- `jp_lit_delete_cache`
  - 指定した cache や tool 単位の cache を明示的に削除する
...
- `jp_lit_prune_cache` は古い原本 cache を確認して掃除する

という流れです。

検索・取得系ツールの返り値に入る `cache.hit=true` は、その呼び出しで保存済み cache を再利用したことを示します。結果整理の原本として使うのは主に `jp_lit_search` の cache ですが、OCR・全文・図版・国書DB本文検索・典拠補助などの cached tool でも、同じ考え方で `cache.saved_at` と再取得導線を確認できます。`jp_lit_refine_results` は既定で整理後の先頭 30 件だけを返します。`include_enrichment=true` を指定した場合も外部 API へは接続せず、保存済み `jp_lit_enrich_record` cache を重複クラスタへ重ねるだけです。
...
```text
過去に保存した「明治期の俳句雑誌」の検索結果も一緒に見て、共通して出るものだけまとめてください。
```
...
- `jp_lit_refine_results` の結果が 30 件を超える場合は、会話では先頭 30 件だけを返します。その際は「全何件中、どの順で、先頭 30 件を返しているか」を明示します。全件が必要なら、そのまま export します。export すると、並び順・絞り込み条件・全件リストを固定したファイルとして扱えるため、あとから同じ内容を読み直したり、別セッションやサブエージェントに渡して傾向要約・重複確認・採用候補の下読みをさせたりできます
- `jp_lit_prune_cache` は既定で削除候補を表示するだけです。実際に消す場合は `dry_run=false` を指定します
- `jp_lit_delete_cache` や `jp_lit_prune_cache(dry_run=false)` で cache を消すと、あとから `unselected` を含む export や再抽出に使える材料が減ります。必要なら先に export してください
- 全件確認が必要な場合は、まず `jp_lit_export_view` で Markdown / JSON に書き出すのが基本です
- Zotero などへ CSL JSON で渡す前は、重複候補を確認してから採用候補だけを書き出す運用をおすすめします
...
重複候補も同じ考え方で扱います。会話内の `jp_lit_refine_results` は既定では重複クラスタを出しません。全件を見比べたい場合は、次のように Markdown / JSON へ書き出してから、採用・除外を確認します。

```text
jp_lit_export_view(
  view="refined_results",
  params={ cache_keys=[...], combine="union", include_enrichment=true },
  export_all=true,
...
```text
# 1. 保存済み検索結果を統合し、重複候補付きで全件 export
jp_lit_export_view(
  view="refined_results",
  params={ cache_keys=[...], combine="union", include_enrichment=true },
  export_all=true,
...
- `cache/` = 原本
- `sessions/` = エージェントの選別ログ、調査経過、短いメモ
...
- 古い検索・取得結果の cache だけ確認して消したい
  - `jp_lit_prune_cache(older_than_days=30)` で候補確認
  - 問題なければ `jp_lit_prune_cache(older_than_days=30, dry_run=false)` で削除
- export したファイルだけ消したい
  - `exports/` 以下の該当ファイルを削除

`jp_lit_prune_cache` は原本キャッシュだけを対象にし、セッションや `exports/` は削除しません。削除しても、次に検索や保存を行えば必要なものは再作成されます。ただし、過去の調査ログや候補メモから参照している cache は失われるため、残したいものは先に export しておくのがおすすめです。
...
公文書・アジア歴史資料では、`national_archives` は国立公文書館DAの官庁資料・特定歴史公文書、`jacar` は外交・軍事・旧外地・植民地関係資料の目録確認に使います。リサーチ・ナビやレファ協がこれらを推奨している場合も追加候補にします。どちらも通常の文献探索の主力 source ではなく、必要になった場面で明示指定する下位導線です。画像本体や OCR 本文は取得せず、公式レコード URL で最終確認します。

---

### 参考書誌・索引・有料DBでの追加確認

`jp-lit-mcp` は、紙の参考書誌・専門索引・年鑑・事典・館内限定資料や、契約が必要な有料DBには直接接続しません。特に、戦後以降の一般誌・週刊誌・総合雑誌・新聞記事・論壇記事・インタビュー・広告・書評欄などは、オープンな source だけでは抜けることがあります。

この領域が調査目的に関係する場合、またはリサーチ・ナビやレファ協が参考書誌・索引・有料DBを勧めている場合、Skill 併用時は `参考書誌確認` / `要有料DB確認` として、次に人間が確認するとよい資料・DB や検索語案を短く出すことがあります。

- 参考書誌・専門索引・年鑑・目録・事典: 分野ごとの文献一覧、総目次、掲載確認
- ざっさくプラス: 雑誌記事、総合雑誌、論壇誌、一般誌の手がかり
- 大宅壮一文庫: 人物、事件、世相、サブカルチャー、週刊誌・大衆誌の記事
- 新聞系 DB: 朝日クロスサーチ、ヨミダス、毎索、日経テレコン等

これは毎回の注意書きではありません。学術論文・図書・デジタル化資料だけで足りる調査では通常出さず、参考書誌・索引・一般誌・新聞・戦後以降のメディア言説が重要そうなときだけ、未確認領域として示します。
...
| ツール                        | 何をするか                                                   | 使いどころ                                 | 注意点                                                                           |
| ----------------------------- | ------------------------------------------------------------ | ------------------------------------------ | -------------------------------------------------------------------------------- |
| `jp_lit_search_fulltext`      | 次世代デジタルライブラリーの OCR 全文を資料横断で検索します  | どの資料に語が出るか探す                   | デジコレ本体の全文検索 API ではありません。対象はインターネット公開資料中心です |
...
| ツール | 何をするか | 使いどころ | 注意点 |
| ------ | ---------- | ---------- | ------ |
| `jp_lit_search_kokusho_fulltext` | 国書DB収録資料の本文スニペットを検索します | 古典籍本文中の語を探す | 本文全体・画像本体・manifest 本体は取得しません |
| `jp_lit_search_kokusho_image_tags` | 国書DBの画像タグを検索します | 古典籍の図像タグ・挿絵モチーフを探す | 画像本体は取得せず、画像パス文字列のみ返します |
...
Skill 併用時、エージェントは候補の確からしさと本文確認状態を分けて整理します。単独の `確認済み` ではなく、候補ごとに `候補確度:`、`確認:`、`本文:` を併記します。

| ラベル | 意味 |
| ------ | ---- |
| `候補確度: 書誌確認済み` | 題名・著者・発行年・永続 ID などが信頼できる DB で確認できたもの。本文読了は意味しません |
| `候補確度: 有力候補` | 書誌や OCR ヒットはあるが、現物・画像・同名別資料などの確認が残るもの |
| `候補確度: 弱い候補` | タイトル類似・OCR 断片・関連語ヒットなど、追加確認が必要なもの |
| `候補確度: 関連性未確定` | 検索結果には出たが、本文・要旨・目次のいずれでも関連性を確認していないもの |

本文または該当箇所を読んだ場合だけ `確認: 本文` / `本文: 確認済み` と表示します。検索結果だけの資料は `確認: 検索ヒットのみ` として保留します。
...
`jp-lit-mcp` では、MCP サーバーが検索・取得の道具を提供し、同梱 Skills が主に調査の進め方を組み立てます。分野や扱う資料によって、先に見るべき DB、検索語の作り方、確認済みと未確認の分け方は変わります。

Skills は調査方針や出力形式を安定させるための補助ですが、エージェントが必ずすべての指示を完全に守るとは限りません。調査ログ、本文確認状態、除外候補、検索語の表記ゆれなどに抜けがありそうな場合は、会話の中で追加指示してください。
...
```text
本文を読んだものと、オンライン入口があるだけのものを分けてください。
```
...
Skills の調査方法に「まず別の DB を見たほうが良い」「この検索語は不自然」「この種類の資料では断定を避けたい」といった違和感があった場合は、[フィードバックの送り方](feedback.md) を参照してください。
