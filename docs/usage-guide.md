...
`優先` は読む順番や確認順を決めるための作業上の目安です。候補はフラットに並べるだけでなく、資料種別、出版社・媒体、シリーズ、著者属性、引用・書評状況、刊行時期、本文確認状況などを見て、`優先: 高` / `中` / `低` / `保留` のように仮に整理することがあります。ただし、出版社や媒体だけで文献の価値を確定しません。個人出版や一般向け新書にも、一次資料性・同時代証言・論争史上の価値がある場合があります。
