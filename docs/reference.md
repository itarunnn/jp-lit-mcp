...
OCR 系ツールはインターネット公開資料のみ対応します。上流は次世代デジタルライブラリー API であり、デジコレ本体（`dl.ndl.go.jp`）の全文検索画面を API として取得するものではありません。デジコレ本体の検索画面では館内限定・送信サービス限定資料の OCR ヒットが見える場合がありますが、`jp_lit_search_fulltext` はそれらを網羅しません。
...
次世代デジタルライブラリー収録資料を OCR 全文テキストから検索します。これはデジコレ本体の全文検索 API ではないため、公式検索画面でヒットする限定公開資料が結果に出ないことがあります。
