...
| `filters.cinii.category` | string | なし | `source=cinii_books` のときのみ。NDC / NDLC notation を半角スペース区切りで渡す |
...
| フィールド | 型 | 説明 |
| ---------- | -- | ---- |
| `category` | string | CiNii Books の `category` パラメータ。NDC / NDLC notation を半角スペース区切りで渡す |

対応 source は `cinii_books` だけです。件名語から分類記号を探す場合は `jp_lit_suggest_classification_codes` を使うと、`suggested_category_param` と `jp_lit_search` 呼び出し例を得られます。
...
したがって、`jp_lit_search_fulltext` の結果だけで「デジコレ本体の全文検索にヒットなし」とは判断しないでください。網羅性が必要な調査では、公式検索画面でのブラウザ検索・手動確認を併用し、MCP で確認した範囲と手動確認した範囲を分けて記録します。
...
次世代デジタルライブラリー収録資料を OCR 全文テキストから検索します。これはデジコレ本体の全文検索 API ではなく、デジコレ本体の「ログインなしで閲覧可能」資料全体を検索するものでもありません。公式検索画面でヒットする資料が結果に出ないことがあります。

0 件の場合も「次世代デジタルライブラリー OCR 検索では 0 件」と扱い、デジコレ本体の公式検索画面を確認していない限り「デジコレ全文検索で 0 件」とは表現しません。
...
| `from` | number | 0 | オフセット |
| `f_ndc` | string | なし | NDC 分類 filter。上位分類は `9*` のような前方一致で指定する。`9` のような1-2桁の数字だけを渡した場合は `9*` に正規化する。空文字・空白だけなら未指定扱い |
...
上流検索語には Illustration API の `keyword2vec` を使います。`q-contents` や `graphictag` は検索パラメータとして採用せず、図版種別を確認する場合は返却後の `graphictags[].tagname` を見ます。
...
Web NDL Authorities で件名語から NDC / NDLC 分類記号を探し、CiNii Books の `category` filter に渡せる `suggested_category_param` を返します。
...
`available=false` は、実務上は次世代側未収録であることが多いですが、アクセス制限や上流都合との厳密な区別はしていません。PID を解決できない場合は `next_digital_library` 自体が `null` になります。個人送信対象などで手動閲覧できる資料は `content_access.manual_viewing` を確認してください。`next_digital_library.available=false` と `manual_viewing.available=true` は両立します。ただし `manual_viewing` は手動閲覧導線であり、MCP がデジコレ本体の全文検索結果や本文 OCR を取得できることを意味しません。
...
- `SMOKE_LOCAL_SOURCE`: local persistence smoke の検索 source。既定は `cinii_books`
- `SMOKE_LOCAL_QUERY`: local persistence smoke の検索語。既定は `夏目漱石`
...
- 次世代デジタルライブラリー API と OCR 系ツールは、次世代デジタルライブラリー収録資料のみ対象です。デジコレ本体の「ログインなしで閲覧可能」資料全体とは一致しません。
