...
この設計では、「選別前」と「選別後」を別レイヤで保持する。

- キャッシュ
  - 各ツールの `structuredContent` 全体を保存する
  - つまり未選別候補を含む正規化済みの元結果を保持する
- セッション
  - キャッシュ済み結果のうち、どの候補を LLM が採用したかを保存する
  - 保持するのは候補ラベルと短いメモであり、元結果全体の複製ではない

たとえば `jp_lit_search` で 48 件取得した場合、48 件全体はキャッシュに残る。
そのうち 3 件を `strong_candidate` と判断したなら、その判断だけがセッションへ残る。
...
ここでいう「取得結果」はキャッシュ参照を意味し、セッション本体には選別後の注釈を中心に残す。
