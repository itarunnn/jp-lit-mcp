...
早見表の「明示的な登録要件は未確認」は、確認した公開情報の範囲では CiNii の App ID のような事前登録要件を見つけていない一方で、公式に「登録不要」と明記された根拠までは押さえていない、という意味です。
...
- デジコレ本体（`dl.ndl.go.jp`）の全文検索画面に出る OCR ヒットは、次世代デジタルライブラリー API の検索対象とは別です。これは館内限定・送信サービス限定資料だけでなく、デジコレ本体ではログインなしで閲覧できるが次世代 API の収録・検索対象ではない資料にも当てはまります。
...
- リサーチ・ナビ本文を API で検索したい場合の公式導線は CRD API 2.0 です。リサーチ・ナビ画面の曖昧検索（`isFuzzy=true`）は、MCP の API source ではなく、人間またはブラウザ確認用の補助リンクとして扱います。
