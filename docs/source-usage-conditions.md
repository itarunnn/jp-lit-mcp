...
この文書は、`jp-lit-mcp` が利用している主な DB / API の利用条件を、実装・ドキュメント・運用確認用に整理したものです。法的助言ではありません。公開・配布・業務利用・共有サーバ運用を行う場合は、必ず元の規約・ヘルプを再確認してください。
...
| `jstage_articles` | 非営利は不要 | 営利は申請必要 | JST 提供である旨表示、J-STAGE へのリンク | API情報の長期保存・大量取得に注意 | 検索は WebAPI、詳細は記事ページ HTML meta の best-effort |
...
- WebAPI 提供情報を機械可読な状態でサーバやクラウド等に 24 時間以上保存・キャッシュしない旨の条件があります。
- J-STAGE 閲覧規約も遵守対象です。記事ページ HTML の詳細取得は WebAPI ではなく通常ページ取得なので、高頻度取得は避け、壊れやすい best-effort として扱います。

運用メモ:

- `jp-lit-mcp` のキャッシュは通常、利用者のローカル環境内に保存されます。ただし、共有サーバ・クラウド・公開サービスとして運用する場合、J-STAGE WebAPI 由来のメタデータを長期保持しない設計、または source 別 TTL / cache 無効化が必要です。
...
- J-STAGE WebAPI 由来の情報は、サーバ / クラウド等で 24 時間以上機械可読な状態で保存・キャッシュしない条件に注意します。共有運用では source 別 TTL や cache 無効化を検討します。
