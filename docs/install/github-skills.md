...
そのため、この repo で `main` に変更を push しただけでは、`gh skill install itarunnn/jp-lit-mcp ...` がすぐ最新 `main` を読むとは限りません。通常利用では release 済みの安定版を入れる前提にし、公開直後の `main` を確認したい場合だけ `jp-lit-research@main` のように version を明示してください。再現性を固定したい場合は、`jp-lit-research@v0.7.8` のようにタグを付けるか、`--pin v0.7.8` を使います。
...
```bash
gh skill install itarunnn/jp-lit-mcp jp-lit-research@v0.7.8 --agent codex --scope user
gh skill install itarunnn/jp-lit-mcp jp-lit-verification@v0.7.8 --agent codex --scope user
```
...
gh skill preview itarunnn/jp-lit-mcp jp-lit-research@v0.7.8
```
