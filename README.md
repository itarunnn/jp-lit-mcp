...
- `CINII_RESEARCH_BASE_URL`
  ...
  - `cinii_research` source の検索 base URL を上書きします。
...
- `source` は `ndl_search`、`ndl_digital`、`cinii_research` を取ります。
- `ndl_digital` は内部的には `dpid=ndl-dl` を付けた NDL Search API を利用します。
- `cinii_research` は初版では `articles` 検索を使います。
...
- `cinii_research` は初版では `articles` 検索に固定しています。
- `cinii_research` の detail は metadata 中心です。タイトルは `publicationName` fallback を使うため、資料種別によっては厳密な論文題名と一致しないことがあります。
