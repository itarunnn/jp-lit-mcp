...
- 公開ツール 12 種・対応 source 14 種・テスト 318 件すべて通過
...
- 検索前に調査計画（使う source・検索語・理由）を提示してユーザーが確認・調整できる
- standard / deep では CRD（レファ協）と NDL リサーチ・ナビを参照してアドバイスを先出しする
...
`文献DBで` または `文献DBを始めます` で Skill が起動した場合、standard / deep ではいきなり検索を始めず、まず次を見て調査計画を組みます。

- レファレンス協同データベースの調べ方マニュアル
- レファレンス協同データベースの類似事例
- NDL リサーチ・ナビの該当分野ページ
...
を組み立て、ユーザーに確認してから検索用 MCP を呼びます。レファ協やリサーチ・ナビは「答えそのもの」ではなく、調査計画を作るための材料として使います。
...
| `jp_lit_export_session` | 調査セッションを書き出す。`full_log` / `selected` / `unselected` を選べる | セッション |
| `jp_lit_find_sessions` | 過去の調査セッションを主題・キーワード・候補タイトル・メモから検索する | セッション |
...
| `ndl_search` | NDL Search 広域（100 機関以上・存在確認・初動向き） | |
...
テスト 318 件すべて通過。`npm test` / `npm run build` / `npm run smoke:mcp` が通る状態を維持。
...
| `ndl_search` | SRU `dcndl`（dpid 未指定） | NDL detail JSON | — | 100 機関以上の広域横断。存在確認・初動向き。CiNii/J-STAGE はハーベスト済みで薄い。nihu_bridge は対象外 |
...
ラウンドロビンモード（source 未指定）に含まれない source は `jp_lit_search(source=...)` で明示指定が必要。NDL モード（広域・初動向き）には `jp_lit_search(source=ndl_search)` を使う。
...
- `ndl_search` は dpid 未指定で NDL Search 参加機関 100 以上を広域横断する入口です。存在確認・初動調査向き。CiNii・J-STAGE はハーベスト済みメタデータのため著者詳細・所蔵・PDF リンクが欠け、nihu_bridge はカバーしません。詳細調査では `ndl_catalog` / `ndl_digital` / `ndl_articles` 等の個別 source を使ってください。
...
調査計画は対話で決める。依頼の intent とドメインから source の候補を組み立てて提示し、ユーザーが「もっと広く」「まずざっくりで」と応じた場合は source を増減して更新する。

スキル構成（`.cursor/skills/jp-lit-research/`）:

| ファイル | 内容 |
|---------|------|
| `SKILL.md` | 調査フロー（Step 1〜2.5〜7）・plan-first 原則 |
...
| `heuristics/advisory-consultation.md` | 調査前情報収集（CRD・リサーチ・ナビ）— intent に応じて実行 |
...
| `heuristics/clarifying-questions.md` | 計画確認（全モード）と問い返し（intent 不明時）のガイダンス |

### jp-lit-verification Skill の起動フレーズ

| フレーズ | 例 |
|---------|-----|
| 「文献検証で」「資料検証で」 | 「文献検証で、次の文章に出てくる文献を確認してください」 |
| 「文献実在性調査を始めます」「資料存在確認を始めます」 | セッション起動型。文章を後から貼る場合に使う |
| 「この文献（資料）は本当にある？」「実在するか確認して」「存在確認して」 | 単体の文献・資料を疑うとき |
| 「文章に出てくる文献（資料）の実在性・存在を確認して」 | 文章と同時に依頼する場合 |
