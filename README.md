...
このコマンドは `Node.js 18` 以上、パッケージバージョン、同梱 Skills、cache / exports ディレクトリの書き込み、`CINII_RESEARCH_APP_ID` の有無を確認します。外部 DB への live API アクセスは行いません。
...
- 調査結果をセッションとして保存し、あとから Markdown / JSON / CSL JSON で書き出す
