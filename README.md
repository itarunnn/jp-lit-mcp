...
このコマンドは `Node.js 18` 以上、パッケージバージョン、同梱 Skills、cache / exports ディレクトリの書き込み、`CINII_RESEARCH_APP_ID` の有無を確認します。`CINII_RESEARCH_APP_ID` は CiNii 系と KAKEN API tool で使います。外部 DB への live API アクセスは行いません。
