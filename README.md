...
この OCR 系機能は、次世代デジタルライブラリー API を使います。デジコレ本体の検索画面に出る館内限定・送信サービス限定資料を含む全文検索結果を、MCP が API として網羅取得するものではありません。

重要: 網羅性が必要な調査では、`jp_lit_search_fulltext` だけで「デジコレ全文にヒットなし」と断定しないでください。デジコレ本体の公式検索画面では、MCP が取得できない館内限定・送信サービス限定資料の全文ヒットが見える場合があります。その場合は、MCP の OCR 検索結果に加えて、公式画面でのブラウザ検索・手動確認を併用し、回答では「MCP の次世代デジタルライブラリー検索範囲」と「デジコレ本体での手動確認範囲」を分けて書きます。
