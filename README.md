...
- `source` は `ndl_search`、`ndl_catalog`、`ndl_digital`、`ndl_articles`、`cinii_research`、`cinii_articles`、`cinii_books` を取ります。
...
- 横断検索では `cinii_research` alias と `ndl_search` 互換 source は含めず、`ndl_catalog` → `ndl_digital` → `ndl_articles` → `cinii_articles` → `cinii_books` の順に source ごとラウンドロビンで結果を混在させます。
