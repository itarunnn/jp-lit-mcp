...
| 深度 | 基準 | 内容 |
|------|------|------|
| `quick` | 「ざっと調べて」「参考程度に」 | 1行で調査計画を提示して確認後、ndl_search（広域・1リクエスト）から始める |
| `standard` | 「調べて」「探して」（明示なし） | CRD・リサーチ・ナビ参照 → 計画提示・確認 → source 別検索 → 選別済み候補を報告 |
| `deep` | 「網羅的に」「論文用に」「本気で」 | CRD・リサーチ・ナビ詳細調査 → 計画提示・確認 → 段階的に複数 source → 全文・ページ・図版まで追う |
...
| `heuristics/advisory-consultation.md` | 調査前情報収集（CRD・リサーチ・ナビ）— standard/deep で実行 |
