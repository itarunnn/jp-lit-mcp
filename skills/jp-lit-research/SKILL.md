...
- タイトル・要旨・目次・学術書評・専門誌書評・出版社紹介・Web 断片から仮整理してよい
...
詳細は [reference/03-evidence-and-output.md](reference/03-evidence-and-output.md) と [heuristics/evidence-grading.md](heuristics/evidence-grading.md) を参照。

結果は **確認済み / 有力候補 / 弱い候補** の3段階で評価し、文献リスト上では **主要候補 / 保留・弱い除外候補 / 除外した代表例** に分けて表示する。OCR ヒットのみで内容を断定しない。

候補を報告するときは、各項目に最低限次を添える。取れない項目は `未確認` / `DB上は未取得` と書く。
...
リスト出力では、候補ごとに項目名や順番を変えない。原則として `種別` / `source` / `確認` / `本文` / `優先` / `根拠` / `仮整理` / `次` の順に揃える。詳細テンプレートは [reference/03-evidence-and-output.md](reference/03-evidence-and-output.md) を参照。

文献リストを返すときは、原則として `検索概要` → `主要候補` → `保留・弱い除外候補` → `除外した代表例` → `今回の確認範囲` の順にする。候補が少ない場合でも、保留候補は単に省略せず、`なし` と理由を短く示す。`除外した代表例` は明確な除外がある場合に出し、ない場合や未点検の場合は検索概要または確認範囲にその状態を書く。

文献リストは、原則として **主要候補** と **保留・弱い除外候補** を分けて出す。採用候補だけに絞ると調査の幅が見えなくなるため、除外理由が強くないもの、本文未確認で判断保留のもの、周辺だが見落とすと困るものは数件示す。

学術誌・紀要・専門誌の書評、書評論文、紹介、批判的検討、リプライは、通常の読者レビューとは別扱いにする。タイトルに「書評」「紹介」が含まれていても機械的に落とさず、論争の受容・批判・論点整理に関わる可能性があれば文献候補として残す。
...
- 除外したものがあればその理由。理由が弱いものは `保留・弱い除外候補` として見せる
- 返答末尾に `今回の確認範囲` を置き、本文確認済み件数・要旨確認件数・書誌確認中心かどうかを明示する
