...
1. 表記ゆれ・旧字を試す
2. 複合語 0 件を不在証明にしない。空白入り query や `AND` が 0 件でも、単独語検索へ戻す
3. 単独語検索と複合語検索を分けて調査ログに残す
4. 単独語でヒットした資料について、highlights、`jp_lit_search_pages`、本文 OCR、ページ画像を順に確認する
5. `searchfield=metaonly` で書誌検索に切り替える
6. より上位語・関連語で検索する
