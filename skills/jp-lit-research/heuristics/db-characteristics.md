...
**制約:** sort 未対応。横断検索のデフォルト対象だが、確実に使いたい場合は `source=nihu_bridge` を明示して呼ぶこと

## nijl_articles
国文学・アーカイブズ学論文DB。国文学論文、国文研論文、日本文学研究論文の専門目録として使う。
**向いている用途:** 古典文学・近世文学・日本文学研究の論文探索、CiNii/NDL だけでは拾いにくい国文学系採録の確認
**制約:** 既定横断検索の対象外。HTML best-effort。本文、PDF、OPAC 詳細追跡は取得しない

## kokusho
国書データベース。国書・古典籍・写本・版本の著作、書誌、所在、画像公開導線を確認する。
**向いている用途:** 古典籍同定、異本・写本・版本の所在確認、著作と書誌の関係確認
**追加 tool:** 本文スニペットは `jp_lit_search_kokusho_fulltext`、画像タグは `jp_lit_search_kokusho_image_tags`
**制約:** 既定横断検索の対象外。manifest URL や license URL は保存するが、manifest 本体、画像本体、本文一括取得はしない。本文スニペットや画像タグは公式画面確認前の手がかり

## ninjal_bibliography
日本語研究・日本語教育文献データベース。日本語研究、日本語教育文献、国語教育文献の専門目録として使う。
**向いている用途:** 日本語学、日本語教育、国語教育の文献探索、分野・キーワードからの候補整理
**制約:** 既定横断検索の対象外。HTML best-effort。本文リンク URL は保存するが、本文 PDF や外部本文は取得しない
