...
次に当てはまる場合は、回答の `次` や `今回の確認範囲` に `参考書誌確認` または `要有料DB確認` を短く残す。
...
毎回の定型注意にはしない。参考書誌・索引・雑誌記事・新聞記事・戦後以降の一般言説が調査目的に効く場合だけ、確認先と検索語案を出す。
...
3. jp_lit_search_fulltext    — OCR 全文横断
4. nihu_bridge               — 人文学専門 DB（国文研・国民博・etc.）
```
...
| `kokusho` | 既定横断外。manifest URL は保持するが、manifest 本体・画像・OCR は対象外 |
