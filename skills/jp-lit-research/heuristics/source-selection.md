...
カーリル図書館MCPが使える環境では、まず `search_libraries` で地域名、館種、ネットワーク名、専門資料機関名を検索し、該当都道府県立図書館、市区町村中央館、県内/広域の図書館ネットワーク、発行地・活動地に対応する中央館、郷土資料室・分館、隣接自治体や旧郡域の館、専門図書館・資料室の `systemid` を確認する。人名はまず出身地・居住地・活動地・郷土人物としての地域を割り、地域から図書館へ進める。Web 検索は、カーリルで見つからない文学館・記念館・資料館・資料室、地域パスファインダー、新聞・雑誌所蔵一覧、郷土資料ページ、閲覧条件の補助確認に使う。その後 `search_books` へ進む。カーリル図書館MCPの `search_books` は最大15館までなので、全国総当たりではなく、地域推定後に候補館を優先づけて使う。Cursor / Claude Code / Codex ではユーザーがカーリル図書館MCPを登録し初回 OAuth 認可済みなら同一エージェントから実行できる。接続できない場合は MCP / OAuth 設定を直し、必要に応じて各館 OPAC、新聞・雑誌所蔵一覧、図書館レファレンス相談を次アクションに残す。
