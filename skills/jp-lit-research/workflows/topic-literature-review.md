...
**初手の推奨 source（調整の出発点）:**
- 広域・入口確認: ndl_search / japan_search
- 論文: cinii_articles / jstage_articles / ndl_articles
- 図書: ndl_catalog / cinii_books
- リポジトリ（紀要・学位論文）: irdb（必要に応じて提案）

初手で実行する source は 2〜4 個に絞る。迷う場合は `ndl_search` と `japan_search` を基礎候補にし、レファ協・リサーチ・ナビで示唆された専門 DB / source と、資料種別に応じた論文・図書系 source を足して優先順位を決める。
レファ協・リサーチ・ナビから得た索引名、参考図書、総目次、データベース名、検索語候補は、MCP の `source` と混同せず、`reference_tools` と `検索語候補` として計画に反映する。
...
提案した source を順番に検索する。以下は候補例であり、初手で全てを実行しない。
