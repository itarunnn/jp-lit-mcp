...
- 例外として、NDL リサーチ・ナビは調査前情報収集のために Web 検索で確認する。ここで得た情報は調査ルートの材料であり、資料本文や書誌確定とは分ける
...
調査前情報収集では、レファ協から類似質問・調査プロセス・参考資料を、リサーチ・ナビから分野別の調査順序・見るべき索引や参考図書を拾い、`first_pass_plan` に反映する。リサーチ・ナビは Web 検索で `site:ndlsearch.ndl.go.jp/rnavi <固有名詞または主題語>` を試し、不発なら調査類型へ抽象化する。参考図書・専門索引・有料 DB が示唆された場合は、無料オンラインで完結しない可能性を前提に `参考書誌確認` / `要有料DB確認` として計画と最終回答に残す。
...
- cache は検索結果・取得 payload の保管場所、session trace は検索試行と判断の機械寄り記録、調査トレース報告 / handoff report は主エージェントが読む人間可読の引き継ぎ報告として分ける
- 断定、引用、candidate の格上げ、競合解消、export 作成時だけ原本へ戻る
- 通常の探索ループ（source 選択、検索語展開、候補判断、次 query の決定）は主エージェントが文脈を持って統合する
- 独立した調査線が 2 本以上ある場合は、サブエージェント分担をデフォルト寄りに検討する。例: `NDL/CiNii の書誌・所蔵確認`、`Japan Search/文化資源確認`、`専門領域 Web 補助確認`、`地域公共図書館・専門資料室導線`
- サブエージェントに任せる場合は、担当範囲と対象 `cache_key` / `session_id`、使ってよい source、調査トレース報告の出力形式を固定する
- 各サブエージェントは自分の担当範囲について、検索試行、採用/保留/除外理由、本文確認範囲、未確認事項、次アクションを trace に残し、主エージェントへ brief summary ではなく調査トレース報告 / handoff report を返す
- 調査トレース報告は current report として更新してよい。追加プロンプトで訂正・追補が入った場合は、本文を最新版に直し、末尾の `変更履歴` に何を直したかを残す
- 主エージェントは handoff report を先に読み、必要な場合だけサブエージェントの trace / cache に戻る。重複・競合・優先度を整理し、最終判断と回答を統合する single writer になる
