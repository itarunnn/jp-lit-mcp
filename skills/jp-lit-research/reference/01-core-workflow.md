...
1. 依頼を intent に分類する
2. 原則として、レファ協・NDL リサーチ・ナビを参考に調査前情報収集を行う。固有名詞 query が不発なら抽象度を上げた調べ方 query を試す
...
調査前情報収集では、レファ協から類似質問・調査プロセス・参考資料を、リサーチ・ナビから分野別の調査順序・見るべき索引や参考図書を拾い、`first_pass_plan` に反映する。参考図書・専門索引・有料 DB が示唆された場合は、無料オンラインで完結しない可能性を前提に `参考書誌確認` / `要有料DB確認` として計画と最終回答に残す。
