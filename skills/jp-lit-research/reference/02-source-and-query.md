...
件名語から未知の図書候補を広げる場合は、`jp_lit_suggest_classification_codes` で NDC / NDLC を取得し、返された `suggested_search.args.filters.cinii.category` を `jp_lit_search(source=cinii_books, ...)` に渡す。分類記号が既に分かっていて件名語を探す場合は `jp_lit_find_authority_terms_by_classification` を使う。分類記号は探索補助であり、候補の主題適合や所蔵は `jp_lit_get_record` と公式ページで確認する。
