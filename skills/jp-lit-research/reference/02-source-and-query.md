...
新規テーマでは、レファ協・NDL リサーチ・ナビを参考に調査計画を立ててから実検索へ進む。実検索では、原則として `source` 未指定の `jp_lit_search(query=...)` から始めない。

実検索の初手は目的に応じて source を明示する。`ndl_search` と `japan_search` は基礎候補だが、固定で専門 DB より先に置くものではない。レファ協・リサーチ・ナビで示唆された source 候補と組み合わせ、初手は 2〜4 source に絞る。
...
追加候補:
- jp_lit_search(source=<調査前情報収集で示唆された専門 DB>, query=...)
```
